...
- **Pull exact formulas from the primary source, don't quote from memory.** When a specific equation, prefactor, or convention from a cited paper (including the arXiv links above) is needed for a quantitative comparison, use WebFetch/WebSearch to retrieve and read the actual paper (or its arXiv abstract/HTML) rather than reconstructing the formula from recollection — state which equation number you're citing. If a paper is paywalled or you can't verify a number with confidence, say so explicitly and report only what you can verify, rather than supplying an unverified coefficient.
