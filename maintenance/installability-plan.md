...
**What's deliberately deferred to a later session**, per the user's explicit direction: reproducing those 2-3 pybinding helper functions as native `kite.lattice`/`kite.custom` functionality, which would let the 3 ARPES files and 3 disorder files convert too; and deciding what to do with `dos_twisted_bilayer.py`, which may not be convertible at all without a much deeper rewrite.
