...
- **`docker-build`**: failed inside the "Build Docker image" step with a bare "exit code 1" — no further detail obtainable without admin log access. Not yet diagnosed; need the user to paste the log, same as they did for macOS.
...
4. `docker-build` not yet touched — waiting on the user to paste its log before attempting a fix, per the same reasoning as the Windows fix (root cause first, not a guess).
