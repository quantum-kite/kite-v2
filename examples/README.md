...
| `afm_zigzag_bands.py` | Standalone plain-numpy exact diagonalization (no KITEx) of a bearded/Klein-terminated graphene ribbon with a fixed staggered Néel mass: bulk bands (Gamma-K-M-Gamma) and ribbon bands, showing the bulk gap and the near-flat edge bands close to E=&plusmn;&Delta;. A quick, cheap check of the model before running the KPM examples below. |
| `afm_zigzag_ribbon.py` | Real-space Sz map (`ldos_map(operators=[...])`, one operator per sublattice) of the same Néel-gapped bearded/Klein ribbon, clean vs. ~5% vacancy concentration (both spin channels removed at the same site). Post-process with `afm_zigzag_ribbon_process.py`. See the [in-depth write-up][afm-ribbon-example]. |
